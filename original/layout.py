...
parser.add_argument('-dpi', type=int, default=600, help='layout export resolution (overridden by atlas config "dpi:")')
...
def getmapxml(name, offset, size, extent, theme, proj4, zindex = 0, custompropertiesandgrids = '', rotation = 0, atlas=True, customattributes = ''):
  ...
  return (f'<LayoutItem uuid="{name}" size="{size},mm" mapFlags="{0 if True else 1}" blendMode="{6 if theme == "mono" else 0}" followPreset="true" position="{offset},mm" zValue="{zindex}" positionOnPage="{offset},mm" type="65639" followPresetName="{theme}" visibility="1" id="{name}" mapRotation="{rotation}" positionLock="true" {customattributes}>{custompropertiesandgrids}<Extent xmin="{extent[0][0]}" xmax="{extent[0][1]}" ymin="{extent[1][0]}" ymax="{extent[1][1]}"/><crs><spatialrefsys><proj4>{proj4}</proj4></spatialrefsys></crs><AtlasMap margin="0" scalingMode="0" atlasDriven="{1 if atlas else 0}"/></LayoutItem>')
...
#  tblabeloptions = 'uuid="{' + getid() + '}" gridStyle="3" showAnnotation="1" annotationFontColor="255,255,255,255" frameAnnotationDistance="' + str(mapmargins[1] - 7.5) + '" annotationFormat="8" annotationExpression="' + labelexpression + '"'
  ...
  newlabeloptions = 'gridStyle="3" showAnnotation="1" frameAnnotationDistance="' + str(mapmargins[1] - labelbgsize - .5) + '" annotationFormat="8" annotationExpression="' + labelexpression + '"' # -.5 is a compromise between top/bottom (which would want -2) and left/right (which would want 0)
...
#      <BackgroundColor red="255" green="255" blue="''' + str(backgroundblue) + '''" alpha="255" />
    ...
    return (f'<LayoutItem size="{size},mm" mapFlags="{0 if inner else 1}" blendMode="{6 if theme == "mono" else 0}" followPreset="true" position="{offset},mm" zValue="{zindex}" positionOnPage="{offset},mm" outlineWidthM="13,mm" type="65639" followPresetName="{theme}" visibility="1" id="{name}" mapRotation="{rotation}" positionLock="true">' + customproperties +
...
def getscaleline(mapUuid, position, unit = 'km', blueonwhite = True, unitsPerSegment = 1, nsegments = None):
  if nsegments == None:
    nsegments = 7 if unit == 'km' else 5
  return f'''<LayoutItem background="false" boxContentSpace="3" frame="false" lineCapStyle="flat" height="{1.5 if blueonwhite else .7}" labelBarSpace="1" labelHorizontalPlacement="0" labelVerticalPlacement="{1 if unit == 'km' else 0}" mapUuid="{mapUuid}" numSegments="{nsegments}" numSegmentsLeft="0" numSubdivisions="1" numUnitsPerSegment="{unitsPerSegment}" outlineWidth="0.33" outlineWidthM="1.33,pt" position="{position},mm" positionLock="false" referencePoint="3" segmentSizeMode="0" size="40,10,mm" style="Line Ticks {'Down' if unit == 'km' else 'Up'}" type="65646" unitLabel="{unitTypes[unit]}" unitType="{unit}" visibility="1" zValue="20">
        ...
        <lineSymbol>
          <symbol alpha="1" force_rhr="0" type="line" name="" clip_to_extent="1">
            <layer enabled="1" locked="0" pass="0" class="SimpleLine">
              ...
              <prop k="line_style" v="{'no' if (blueonwhite and unit == 'km') else 'solid'}"/>
...
#      return '<LabelFont style="Medium" description="Avenir Next Condensed,' + str(fontsize) + ',-1,5,57,0,0,0,0,0,Medium"/><FontColor blue="255" green="255" red="255" alpha="255"/>'
      if fontsize >= 28:
        # big fat title
        return f'<LabelFont description="Helvetica Neue LT Std,{fontsize},-1,5,81,0,0,0,0,0,83 Heavy Extended" style="83 Heavy Extended"/><FontColor blue="255" green="255" red="255" alpha="255"/>'
      elif fontsize >= 18:
...
#        return '<LabelFont style="Condensed Regular" description="HelveticaNeue Condensed,' + str(fontsize) + ',-1,5,50,0,0,0,0,0,Condensed Regular"/><FontColor blue="255" green="255" red="255" alpha="255"/>'
  ...
  def bluetextbox(id, size, position, labelText, fontsize = 14, opacityCondition = None, rotation = 0, referencePoint = 4, blueonwhite = False, frame = False, attributes = 'valign="128" marginX="0" marginY="0"', html = False):
...
#  overviewscale = mapscale * 12.5
  overviewscale = 320000 # ugly kunming hack
...
#  atlasboxheight = innermapsize[1] * mapscale / 1000
  ...
  overviewlayout += bluetextbox('title', sizespecstring([ outermapsize[0] / 2, 18 + args.bleed ]), sizespecstring([ args.papersize[0] / 2, outermargins[1] ]), f'Z-A {layoutname.upper()}', 28, referencePoint=0, attributes = ' valign="64" marginX="0" marginY="1"') # SOME offset from bottom
  # mode="1" for raster
  ...
  # add dummy map with correct scale (1x1cm map with some wobbly extent)
  overviewlayout += getmapxml('scale dummy', '0,0', '10,10', [[0, mapscale/100], [0, mapscale/100]], 'blank', getcrs(config).to_proj4())
  # set frame True for a black frame, add 1mm extra on each side so it's definitely cut off (by pdf cutting) on the left
  overviewlayout += bluetextbox('firstpagescale', sizespecstring([ outermapsize[0] / 2 + 2, 19 ]), sizespecstring([ args.papersize[0] / 2 - 1, 155 ]), f'&lt;div style=&quot;text-align:left;&quot;>&lt;span style=&quot;display:block;margin-top:0.4em;margin-left:2.5em;&quot;>SCALE&lt;/span>&#xa;&lt;span style=&quot;display:block;font-weight:300;font-size:45%;margin-left:20rem;margin-top:-1.8rem;&quot;>Map Pages 4-213 &amp;nbsp;&amp;nbsp;&amp;nbsp;&amp;nbsp;1: {format(int(mapscale), ",")}&lt;/span>&#xa;&lt;span style=&quot;display:block;font-weight:300;font-size:45%;margin-top:3.5rem;margin-left:20rem;&quot;>{round(100000/mapscale, 2)} cm to 1 km&lt;/span>&lt;/div>', 13, referencePoint=0, frame = True, html = True) # valign scale label at the top
  overviewlayout += getscaleline('scale dummy', sizespecstring([ args.papersize[0] * 3 / 4 - 30 , 166 ]), 'm', False, 250, 4) # 3 in the original
  overviewlayout += bluetextbox('copyleft', sizespecstring([ outermapsize[0] / 2 + 2, 19 ]), sizespecstring([ args.papersize[0] / 2 - 1, 174 ]), "&lt;span style=&quot;display:block;font-size:14pt;color:black;&quot;>All Wrongs Reserved by The Futile Publishing Company&lt;/span>", 13, referencePoint=0, blueonwhite=True, html=True)
  # 🄯
  ...
  overviewlayout += bluetextbox('key', '44,6', '78,242', 'KEY TO MAP PAGES', fontsize = 11, blueonwhite = True)
  overviewlayout += bluetextbox('scalebox', '44,17', '61,401', 'SCALE', fontsize = 8, blueonwhite = True)
  overviewlayout += getscaleline(layoutname, '38,400', 'mi') # miles scaleline
  overviewlayout += getscaleline(layoutname, '38,404.7', 'km') # km scaleline
...
if args.write:
  ...
  def addlayout(newlayout):
    print(f'adding layout {newlayout.get("name")} to ZA2.qgs')
    existing = layouts.find(f"./Layout[@name='{newlayout.get('name')}']")
    if existing:
